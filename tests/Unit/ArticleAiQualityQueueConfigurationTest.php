...
class ArticleAiQualityQueueConfigurationTest extends TestCase
{
    ...
    public function test_production_deploy_and_healthcheck_require_quality_workers(): void
    {
        ...
        $this->assertStringContainsString(
            'up -d --remove-orphans app web queue ai-quality-queue ai-quality-backfill-queue knowledge-queue scheduler reverb',
            $deploy,
        );
        $this->assertStringContainsString(
            'stop web app queue ai-quality-queue ai-quality-backfill-queue knowledge-queue scheduler reverb',
            $deploy,
        );
        ...
        $this->assertStringContainsString(
            'required=(postgres redis app web queue ai-quality-queue ai-quality-backfill-queue knowledge-queue scheduler reverb)',
            $healthcheck,
        );
        $this->assertStringContainsString(
            'logs --tail=80 app queue ai-quality-queue ai-quality-backfill-queue knowledge-queue scheduler web',
            $healthcheck,
        );
        ...
        foreach (['README_en.md', 'README_es.md', 'README_ja.md', 'README_ru.md'] as $readme) {
            ...
            $this->assertStringContainsString(
                'up -d app web queue ai-quality-queue ai-quality-backfill-queue knowledge-queue scheduler reverb',
                $contents,
            );
        }
    }
